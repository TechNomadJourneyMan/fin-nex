...
const String kFnxDatabaseFilename = 'finnex.db';
...
class FnxDatabase {
  ...
  /// location.
  static Future<FnxDatabase> open({
    String filename = kFnxDatabaseFilename,
  ...
  static Future<FnxDatabase> openInMemory({
  ...
  static Future<FnxDatabase> _openWith(
    DatabaseFactory factory,
    String path, {
  ...
  static Future<void> _onCreate(
    ...
    for (final stmt in FnxSchema.createStatements) {
      batch.execute(stmt);
    }
    await batch.commit(noResult: true);

    if (seedSystemData) {
      await FnxSeeder.seedSystemCategories(db);
    }
  }
  ...
  static Future<void> _onUpgrade(
    ...
    debugPrint('FnxDatabase migrate $oldVersion -> $newVersion (no-op)');
  }
