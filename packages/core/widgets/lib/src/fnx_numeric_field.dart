...
import 'fnx_text_field.dart';

/// Input formatter that groups digits with locale separators while typing.
class FnxThousandsFormatter extends TextInputFormatter {
  /// Creates a thousands formatter for [locale].
  FnxThousandsFormatter({this.locale = 'ru'});
...
class FnxNumericField extends StatelessWidget {
  /// Creates a numeric field.
  const FnxNumericField({
    super.key,
    this.label,
    this.hint,
    this.controller,
  ...
  Widget build(BuildContext context) {
    return FnxTextField(
      ...
      inputFormatters: [FnxThousandsFormatter(locale: locale)],
      onChanged: (value) {
