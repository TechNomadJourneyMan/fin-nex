// Theme convenience extension for FinNex widgets.
//
...
//
// IMPORTANT: The public token classes (`FnxColors`, `FnxSpacing`,
// `FnxRadius`, `FnxTypography`, etc.) live in `package:fnx_core_tokens` and
...
// its semantic helpers under distinct names (`FnxSemanticColors`,
// `FnxSemanticSpacing`, `FnxSemanticTypography`) and `FnxRadii` (a separate
// helper from the tokens' `FnxRadius`). Widgets consume them via the
// `context.fnxColors`, `context.fnxSpacing`, `context.fnxRadii`, and
// `context.fnxTypography` extensions below.

import 'package:flutter/material.dart';

/// Semantic color palette for FinNex widgets.
///
/// Distinct from `FnxColors` in `fnx_core_tokens` (which is a Material
/// `ThemeExtension`). This type exposes the field shape that widgets in
/// this package rely on (e.g. `brand`, `surface`, `income`).
class FnxSemanticColors {
  /// Build a semantic palette.
  const FnxSemanticColors._({
  ...
  static const FnxSemanticColors light = FnxSemanticColors._(
  ...
  static const FnxSemanticColors dark = FnxSemanticColors._(
...
///
/// Distinct from `FnxSpacing` in `fnx_core_tokens` (which is a Material
/// `ThemeExtension` with `xN` constants).
class FnxSemanticSpacing {
  /// Default const constructor.
  const FnxSemanticSpacing();
...
///
/// Note: this is *not* `FnxRadius` from `fnx_core_tokens`; widgets in this
...
/// Semantic typography styles for FinNex widgets.
///
/// Distinct from `FnxTypography` in `fnx_core_tokens` (which is a Material
...
class FnxSemanticTypography {
  /// Build a typography set from a [TextTheme].
  const FnxSemanticTypography(this._base, this._textColor);
...
extension FnxThemeExt on BuildContext {
  /// Semantic color palette for the current brightness.
  FnxSemanticColors get fnxColors {
    final brightness = Theme.of(this).brightness;
    return brightness == Brightness.dark
        ? FnxSemanticColors.dark
        : FnxSemanticColors.light;
  }

  /// Spacing scale.
  FnxSemanticSpacing get fnxSpacing => const FnxSemanticSpacing();

  /// Radii scale.
  FnxRadii get fnxRadii => const FnxRadii();

  /// Typography scale.
  FnxSemanticTypography get fnxTypography =>
      FnxSemanticTypography(Theme.of(this).textTheme, fnxColors.textPrimary);
}
