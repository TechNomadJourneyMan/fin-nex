...
class FnxHeatmapCalendar extends StatelessWidget {
  /// Default constructor.
  const FnxHeatmapCalendar({
    super.key,
    required this.from,
    required this.to,
    ...
    this.tileSize = 32,
    this.gap = 4,
    this.baseColor = FnxChartPalette.indigo,
    this.onDayTap,
    this.maxValue,
  ...
  Widget build(BuildContext context) {
    ...
    if (!end.isAfter(start) && !_sameDay(start, end)) {
      return FnxChartEmpty(height: tileSize * 7 + gap * 6);
    }
  ...
  Widget _buildCell(
    ...
    final Widget tile = Container(
      ...
      decoration: BoxDecoration(
        color: color,
        borderRadius: BorderRadius.circular(FnxTokens.radiusSm / 2),
      ),
    );
