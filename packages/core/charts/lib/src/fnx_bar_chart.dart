...
/// One bar group (period bucket) for [FnxBarChart].
class FnxBarPoint {
  /// Default constructor.
  const FnxBarPoint({
    required this.label,
...
/// is shown.
class FnxBarChart extends StatelessWidget {
  /// Default constructor.
  const FnxBarChart({
    super.key,
    required this.data,
    ...
    this.barWidth = 16,
    this.groupSpace = 12,
    this.incomeColor = FnxChartPalette.mint,
    this.expenseColor = FnxChartPalette.indigo,
    this.onBarTap,
  });

  /// Bar points in display order.
  final List<FnxBarPoint> data;
  ...
  Widget build(BuildContext context) {
    ...
    final bool hasIncome = data.any((FnxBarPoint p) => p.income > 0);
    final bool hasExpense = data.any((FnxBarPoint p) => p.expense > 0);
  ...
  double _computeMaxY() {
    double maxV = 0;
    for (final FnxBarPoint p in data) {
