...
import 'pf_chart_legend.dart';
import 'pf_chart_palette.dart';

/// A stacked bar: one X-axis bucket with `n` stacked category values.
class PfStackedBarPoint {
  /// Default constructor.
  const PfStackedBarPoint({required this.label, required this.values});
...
class PfStackedBarChart extends StatelessWidget {
  /// Default constructor.
  const PfStackedBarChart({
    super.key,
    required this.data,
  ...
  final List<PfStackedBarPoint> data;
  ...
  Widget build(BuildContext context) {
    ...
    Color colorFor(String cat, int i) =>
        categoryColors[cat] ?? PfChartPalette.at(i);
  ...
  double _computeMaxY(List<String> shown, List<String> hidden) {
    double maxV = 0;
    for (final PfStackedBarPoint p in data) {
      double sum = 0;
